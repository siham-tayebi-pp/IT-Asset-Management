...
export async function GET(request: NextRequest) {
    try {
        ...
        for (const field of possibleFields) {
            try {
                const searchUrl = `${GLPI_URL}/apirest.php/search/Peripheral?criteria[0][field]=${field}&criteria[0][searchtype]=equals&criteria[0][value]=${userId}`

                console.log(`🖱️ [Peripheral] Tentative avec champ ${field} pour userId ${userId}`)
                ...
                if (response.ok) {
                    ...
                    if (responseData.data && responseData.data.length > 0) {
                        console.log(`✅ [Peripheral] Succès avec champ ${field}, trouvé ${responseData.data.length} périphériques`)
                        data = responseData
                        break
                    } else {
                        console.log(`⚠️ [Peripheral] Champ ${field} OK mais aucun résultat`)
                    }
                } else {
                    const errorText = await response.text()
                    console.log(`❌ [Peripheral] Champ ${field} échoué: ${response.status} - ${errorText}`)
                    lastError = `Field ${field}: ${response.status} - ${errorText}`
                }
            } catch (error) {
                console.error(`💥 [Peripheral] Erreur avec champ ${field}:`, error)
